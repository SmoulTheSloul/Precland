...
cameraMatrix   = np.loadtxt(calib_path+'cameraMatrix800x600.txt', delimiter=',')
cameraDistortion   = np.loadtxt(calib_path+'cameraDistortion800x600.txt', delimiter=',')
##
...
def lander():
    ...
    if vehicle.mode!='POSHOLD':
        vehicle.mode=VehicleMode("POSHOLD")							##Another Mavlink message to MissionPlanner "Landing on Aruco [ID]"
        while vehicle.mode!='POSHOLD':
            print('WAITING FOR DRONE TO ENTER POSHOLD MODE')                                
            time.sleep(1)
    try:
        if ids is not None and ids[0] == id_to_find:
            ...
            if vehicle.mode!='POSHOLD':
                ...
                while vehicle.mode!='POSHOLD':
                    time.sleep(1)
                print("------------------------")
                print("Forcing Messages into Cube")
                print("------------------------")
